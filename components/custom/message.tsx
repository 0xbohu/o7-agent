                    ...
                    <div key={toolCallId}>
                      {toolName === 'getWeather' ? (
                        ...
                        <div className="border rounded bg-muted/50">
                          <button
                            type="button"
                            className="w-full text-left px-2 py-1 font-mono text-xs bg-muted hover:bg-muted-foreground/10 rounded-t flex items-center justify-between"
                            onClick={() => setShowJson((prev) => ({
                              ...prev,
                              [toolCallId]: !prev[toolCallId],
                            }))}
                          >
                            <span>{showJson[toolCallId] ? 'hide' : 'show'} source</span>
                            <span>{showJson[toolCallId] ? '▲' : '▼'}</span>
                          </button>
                          {showJson[toolCallId] && (
